...
<html>
    <head>
		...
		<title>{{ __('Administration Panel') }} - {{ __('View Company') }} #{{$data->companyid}}</title>
    </head>
    <body class="bg-neutral-50">
        ...
        <div class="flex flex-row bg-neutral-50 border b-l-1">
            <div class="basis-1/7 mr-2 flex flex-col">
                ...
                src="@if(file_exists('storage/avatars/'.$data->userid.'.jpg'))
                    ...
                    {{ asset('no-image.png') }}
                    @endif">
                <a href="{{ route('admin.deletecompanypicture', $data->userid) }}" class="mb-6 text-center font-bold text-gray-700 underline text-sm cursor-pointer hover:text-emerald-700">{{ __('Delete Picture') }}</a>
            </div>    
            <div class="basis-5/7 shrink-0 pt-12 flex flex-col justify-center">
                ...
                <p class="text-gray-700 font-bold pb-6">{{ __('Created at') }}: <span class="font-normal">{{ $data->created_at }}</span></p>
            ...
            <div class="basis-1/7 grow-0 shrink-0 ml-auto mr-24 flex flex-col items-center justify-center">
                <a href="{{ route('admin.editcompany', $data->companyid) }}" class="block font-bold text-white text-center p-2 bg-gray-800 text-xl border-0 rounded-md border-gray-800 hover:bg-emerald-800 
                    ease-in-out duration-300 cursor-pointer select-none mb-4 w-48">{{ __('Edit Company') }}</a>
                <a href="{{ route('admin.deletecompany', $data->companyid) }}" class="block font-bold text-white text-center p-2 bg-gray-800 text-xl border-0 rounded-md border-gray-800 hover:bg-red-800 
                    ease-in-out duration-300 cursor-pointer select-none mb-4 w-48">{{ __('Delete Company') }}</a>
                <a href="{{ route('show.admin') }}" class="block font-bold text-white text-center p-2 bg-gray-800 text-xl border-0 rounded-md border-gray-800 hover:bg-blue-800 
                    ease-in-out duration-300 cursor-pointer select-none w-48">{{ __('Manage Site') }}</a>
            </div>
        </div>
        ...
        <div class="flex flex-col border b-r-1 b-b-1 min-h-screen bg-neutral-50">
            ...
            {{ __('There are no job offer entries. Add one') }} <span onclick="showOfferForm()" class="self-center mt-4 font-bold text-gray-700 underline cursor-pointer hover:text-emerald-700">{{ __('here') }}</span>!
            </p>
            @else
            <span onclick="showOfferForm()" class="self-center mt-4 font-bold text-gray-700 underline text-sm cursor-pointer hover:text-emerald-700">+ {{ __('Add New') }}</span>
            ...
            <div class="grid grid-cols-1 auto-cols-auto mt-12">
                <div class="break-all text-center py-4 mr-6 ml-1 mb-6">
                    ...
                    <div>
                        <a href="/listing/{{ $offer->offerid }}" class="mt-4 font-bold text-gray-700 underline text-sm cursor-pointer hover:text-emerald-700">{{ __('View') }}</a>
                        <a href="/offer/edit/{{ $offer->offerid }}" class="mt-4 font-bold text-gray-700 underline text-sm cursor-pointer hover:text-emerald-700">{{ __('Update') }}</a>
        ...
        <span class="material-icons self-end pb-8 cursor-pointer" onclick="closeOfferForm()">close</span>
            ...
            <div class="flex flex-row justify-between pb-4">
                <label for="category" class="font-bold text-gray-700 text-md pr-2">{{ __('Category') }}: </label>
                <input type="text" name="category" id="category" class="border border-neutral-300 w-72">
            </div>

            <div class="flex flex-row justify-between pb-4">
                <label for="location" class="font-bold text-gray-700 text-md pr-2">{{ __('Location') }}: </label>
                <input type="text" name="location" id="location" class="border border-neutral-300 w-72">
            </div>
            
            <div class="flex flex-row justify-between pb-4">
                <label for="salary" class="font-bold text-gray-700 text-md pr-2">{{ __('Salary') }}: </label>
                <input type="number" name="salary" id="salary" class="border border-neutral-300 w-72">
            </div>

            <div class="flex flex-row justify-between pb-4">
                <label for="workload" class="font-bold text-gray-700 text-md pr-2">{{ __('Workload') }}: </label>
                <input type="text" name="workload" id="workload" class="border border-neutral-300 w-72">
            </div>

            <div class="flex flex-row justify-between pb-4">
                <label for="description" class="font-bold text-gray-700 text-md pr-2">{{ __('Description') }}: </label>
            ...
            <div class="flex flex-row justify-between pb-4">
                <label for="extra_info" class="font-bold text-gray-700 text-md pr-2">{{ __('Extra information') }}: </label>
